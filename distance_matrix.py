...
def embed(tcr, v_gene, amino_pos_to_num, max_length, v_dict):
...
def create_distance_matrix(run_number, device):
    ...
    ae_dict = torch.load('vae_vgene_ae.pt', map_location=torch.device(device))
    ...
    for tcr, _ in outliers.keys():
        cdr3 = tcr.split('_')[0]
        vgene = tcr.split('_')[1]
        embeddings.append(embed(cdr3, vgene, ae_dict['amino_pos_to_num'], ae_dict['max_len'], ae_dict['v_dict']))
    encodings = []
