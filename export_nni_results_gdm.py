...
def get_reported_results(trial_log_file):
    minimum_auc_val = []
    try:
        for line in trial_log_file:
            if "Minimum Validation and Train Auc" in line:
                x = line.split(" ")
                minimum_auc_val.append(float(x[-1].replace("\n", "")))
        mean_val = np.mean(minimum_auc_val)
        std_val = np.std(minimum_auc_val)
    except:
        mean_val, std_val = -1, -1
    mean_test, std_test = -1, -1
