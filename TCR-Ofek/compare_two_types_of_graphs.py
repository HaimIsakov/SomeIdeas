...
for i in range(20):
    ...
    df_corr_values = np.array(df_corr_binary.values).astype(int)
    df_proj_values = np.array(df_proj_binary.values).astype(int)

    # G_corr = nx.from_numpy_matrix(df_corr_binary.values)
    # G_proj = nx.from_numpy_matrix(df_proj_binary.values)
